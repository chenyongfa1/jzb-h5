  ...
  export default {
    ...
    methods: {
      ...
      getDetail(level, social, start, end, payType) {
        let type = ""
        if (this.ifBaseSize <= this.cityInfo.social_min_money) {
          this.ifBaseSize = this.cityInfo.social_min_money
        } else if (this.ifBaseSize >= this.cityInfo.social_max_money) {
        ...
        $.ajax({
          ...
          success: function (r) {
            ...
            if(payType==2){
              //此处存值给社保和公积补缴金明细页面
              window.localStorage.setItem('bushebaoMonth',JSON.stringify(Object.keys(r.data).slice(0,-3)))
            }else {
              //此处存值给社保和公积金明细页面
              Object.keys(r.data).slice(0,-2)
              window.localStorage.setItem('shebaoMonth',JSON.stringify(Object.keys(r.data).slice(0,-2)))
