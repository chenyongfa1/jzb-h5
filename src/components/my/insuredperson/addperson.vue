  ...
  <div class="addperson">
      ...
      <div class="updownID pd16">
        <div class="upload">
          <div class="updownId1">
            ...
            <img class="initimg" :src="Img+addJson.id_front" alt="">
          </div>
          <div class="updownId2">
            ...
            <img class="initimg" :src="Img+addJson.id_back" alt="">
          </div>
        </div>
    ...
    <van-popup v-model="isShow" position="bottom" :overlay="true">
      <van-picker show-toolbar
            :default-index="0" @cancel="cancel" @confirm="emergencyConfirm" :title="title"
                  :columns="columns"/>
    </van-popup>
    <van-popup
      v-model="isWorkType"
          ...
          inidata(){
            ...
            $('.updownId2 .shot').css("display", "none")
             $.ajax({
              ...
              dataType: "JSON",
              success: function(r) {
                that.addJson.id_front = r.data.id_front
                that.addJson.id_back = r.data.id_back
                that.username = r.data.name;
                that.sex = r.data.sex
                that.phone = r.data.phone_no
                that.id_no = r.data.id_no
                that.agriculture = r.data.id_type
                that.id_info = r.data.id_info
                that.nationId = r.data.nation
                that.marriage = r.data.marry
                // that.workType = r.data.work_type
                that.emergency_contact = r.data.emergency_contact
                that.emergency_phone = r.data.emergency_phone
                that.social_no = r.data.social_no
                that.accumulation_no = r.data.accumulation_no
                that.nation_name = r.data.nation_name
                that.work_name = r.data.work_name
                that.addJson.nationId = r.data.nation
                that.addJson.workId = r.data.work_type
                // addParticipants = r.data.addJson
                // console.log(that.addJson)
              }
            });
          },
            afterReadid1(file) {
                ...
                $('.updownId1 .uploadidbg').css("display", "none")
                $('.updownId1 .shot').css("display", "none")
                $('.updownId1 .initimg').css("display", "none")
                let that = this
            ...
            afterReadid2(file) {
                ...
                $('.updownId2 .uploadidbg').css("display", "none")
                $('.updownId2 .shot').css("display", "none")
                $('.updownId2 .initimg').css("display", "none")
                let that = this
                this.$ajax.post(this.HOST+'/app/common/upload_file', fd, {
                    headers: {
                        'Content-Type': 'multipart/form-data'
